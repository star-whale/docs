...
|------|--------|-------|-----------|-----|-----------|
|`--name`|`-n`|N|String||数据集的名称，若不指定，则选用dataset.yaml中name字段，若也没有设定，则使用父目录的名字|
|`--handler`|`-h`|N|String||数据集构建的Python入口点，格式为: ${module路径}:${类名或函数名}|
|`--project`|`-p`|N|String|`swcli project select`命令设定的project|Project URI|
|`--dataset-yaml`|`-f`|N|String|dataset.yaml|建议使用默认的dataset.yaml，无需修改。|
|`--append`|`-a`|N|Boolean|False|当指定该参数时，表示此次数据集构建会继承 `--append-from`版本的数据集内容，实现追加数据集的目的。|
|`--append-from`|`-af`|N|String|latest|与 `--append` 参数组合使用，指定继承数据集的版本，注意此处并不是Dataset URI，而是同一个数据集下的其他版本号或tag，默认为latest，即最近一次构建的版本。|
|`--runtime`|`-r`|N|String||`--runtime`参数为Standalone Instance中的Runtime URI。若设置，则表示数据集构建的时候会使用该Runtime提供的运行时环境；若不设置，则使用当前shell环境作为运行时。设置`--runtime`参数是安全的，只在build运行时才会使用Runtime，不会污染当前shell环境。|
|`--desc`||N|String||Dataset的描述|
|`--alignment-size`|`-as`|N|String|4K|swds-bin格式的数据集每个数据块的数据alignment大小，如果设置alignment_size为4k，数据块大小为7.9K，则会补齐0.1K的空数据，让数据块为alignment_size的整数倍，提升page size等读取效率|
|`--volume-size`|`-vs`|N|String|64MB|swds-bin格式的数据集每个data文件的大小。当写数字时，单位bytes；也可以是数字+单位格式，如64M, 1GB等|
|`--data-mime-type`|`-dmt`|N|String|x/undefined|全局默认的数据MIME类型，候选值为 `starwhale.MIMEType` [枚举值](data_type.md)|
...
|参数|参数别名|必要性|类型|默认值|说明|
|------|--------|-------|-----------|-----|-----------|
|`--dest-local-project`|`-dlp`|N|String|当从Cloud Instance向Standalone Instance拷贝数据集时，指定的目标Project，若不设置则通过DEST uri进行推断|Project URI|
...
|参数|参数别名|必要性|类型|默认值|说明|
|------|--------|-------|-----------|-----|-----------|
|`--show-details`||N|Boolean|False|指定该参数后会输出对比的详细信息，包括每行数据的差异，可能会输出较多内容。|
...
|`--show-removed`|`-sr`| N |Boolean|False|显示本地已经删除但能恢复的数据集。|
...
|参数|参数别名|必要性|类型|默认值|说明|
|------|--------|-------|-----------|-----|-----------|
|`--rows`|`-n`|N|Integer|5|显示前N条数据|
|`--show-raw-data`|`-d`|N|Boolean|False|获取Data的数据内容，为bytes类型|
